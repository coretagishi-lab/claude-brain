...
def poll_task(task_type, items):
    for item in items:
        ...
        if task_type == "台本確認":
            ...
            has_override = " (やり直し指示あり)" if yarinaoshi else ""
            print(f"[{ts}] [動画確認] 検知{has_override}: {task_title}", flush=True)
            m = re.search(r"/p/([0-9a-f]{32})", summary)
            content_page_id = ""
            if m:
                raw = m.group(1)
                content_page_id = f"{raw[:8]}-{raw[8:12]}-{raw[12:16]}-{raw[16:20]}-{raw[20:]}"
            if yarinaoshi:
                detail = f"content_page_id:{content_page_id}\n{summary}\nやり直し指示: {yarinaoshi}"
                subprocess.run([
                    "python3", REPORTER,
                    ...
                    "--detail", detail,
                    "--action", "Mac Claude Code: video-generator.py で動画を再生成する",
                    "--source", "dmm-notion-watcher",
                ], timeout=15)
            else:
                detail = f"content_page_id:{content_page_id}\n{summary}"
                subprocess.run([
                    ...
                    "--detail", detail,
                    "--action", "Mac Claude Code: youtube-uploader.py で投稿する（コメントは5分後）",
                    "--source", "dmm-notion-watcher",
                ], timeout=15)
