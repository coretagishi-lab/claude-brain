...
ELEM_TITLE3 = "PBs1sTlCLqHDSG14-LBrqdhnZYLPRPyJX"   # ページ1 タイトル③
...
- テロップ ({len(telops)}行): {json.dumps(telops, ensure_ascii=False)}
...
## 手順 3: タイトル書き換え（ページ1）
`perform-editing-operations` で以下を実行:
- element_id: {ELEM_TITLE3}
- 操作: update_text
- テキスト: "{props['manga_title']}"

## 手順 4: 画像を Canva にアップロード
`upload-asset-from-url` で以下の URL から画像をアップロードし asset_id を取得:
URL: {public_image_url}

## 手順 5: 画像をページ2〜9に挿入
各ページの画像スロットに手順4で取得した asset_id を設定してください。

まず `get-design-pages` でページ一覧を確認し、各ページの画像要素を特定してください。
ページ2の既知スロット ID: {IMAGE_SLOT_P2}

各画像スロットに対して `perform-editing-operations` で:
- 操作: update_fill (fill_type: IMAGE, asset_id: <取得したasset_id>)
- 操作: resize_element でスロットサイズ(954x837)に合わせてリサイズ

## 手順 6: テロップ書き換え（ページ2〜9）
