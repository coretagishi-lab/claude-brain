...
def save_canva_job(props: dict, public_image_url: str, telops: list,
    ...
    comic_frames の使い方（Claude Code セッション側）:
      1. public_image_url の画像をダウンロードして各テロップとコマ内容を照合
      2. telops[i] に最も合うコマ番号（1〜6）を決定して comic_frames[i] に記録
      3. 各スロットで:
           update_fill(element_id, asset_id)
           resize_element(element_id, frame_zoom_params.scaled_img_w, frame_zoom_params.scaled_img_h)
           position_element(element_id, frame_zoom_params.frames[comic_frames[i]].elem_top,
                                        frame_zoom_params.frames[comic_frames[i]].elem_left)
