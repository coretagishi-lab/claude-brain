...
## 1投稿あたりのバリエーション数
- **1 Discord投稿 → 2本（①と②）を自動生成**
- ① = 次の空き枠（今週）
- ② = ①から7日後以降の空き枠（来週）
- 将来的にバリエーション数を増やす場合はdmm-discord-watcher.pyのSLOT_VARIATIONSで制御

## 投稿スロット
- **8:00 JST と 21:00 JST の2枠/日**
- 毎回 ±15〜30分のランダムずらし（パターン検知回避）
- アップロード自体は何時でもOK（YouTubeのscheduledPublishTimeで時刻管理）

## BANリスク対策の優先順位
1. **コンテンツの差別化**（最重要）: 台本・声・サムネイルを変える
2. **投稿タイミングの分散**: 7日間隔・スロットのランダムずらし
3. **同一IPは問題なし**: 同じWiFiからの複数アカウント投稿はYouTubeが許容

---

# 13. 複数アカウント運用時の追加対策【将来実装】
